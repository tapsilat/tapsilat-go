...
func TestMarketplaceAdminPanelSDKStack(t *testing.T) {
	...
	base := tapsilat.MarketplaceSubmerchantCreateRequest{Locale: "tr", ConversationID: "stack-create", Name: "SDK Seller",
		...
		TaxNumber: "1234567890", ContactName: "Contact", ContactSurname: "Person", IdempotencyKey: "sdk-independent"}
	...
	require.Error(t, err)
	t.Log("real SDK HTTP -> Panel gRPC -> Admin domain: independent/selected/multiple create, replay, contracts, account retrieve/environment guard, profile revision, payout materialization, event/replay, gated approval, policy read, invalid token passed; provider and auth fixtures are isolated")
}
